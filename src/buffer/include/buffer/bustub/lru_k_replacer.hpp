...
public:
  static size_t const MAX_K_DISTANCE = ULONG_MAX;
  static size_t const LOW_PRIORITY_TS = 0;

  LRUKNode() = default;
  explicit LRUKNode(size_t ts, size_t k, frame_id_t fid)
      : history_{ts}, k_(k), fid_(fid) {}
  virtual ~LRUKNode() = default;

  auto Filled() const -> bool { return history_.size() == k_; }
  void LowerPriority() { history_.front() = LOW_PRIORITY_TS; }
  ...
  auto CheckEvictable() const -> bool { return is_evictable_; }

  void SetEvictable(bool evictable) { is_evictable_ = evictable; }

  auto GetKDistance(size_t current_ts) const -> size_t {
    if (history_.size() < k_) {
      return MAX_K_DISTANCE;
    }

    return current_ts - history_.front();
  }

  auto GetRecentAccess() const -> size_t { return history_.front(); }

  auto IsHot() const -> bool { return history_.size() >= k_; }
};
...
public:
  ...
  /** record access and pin it in replacer */
