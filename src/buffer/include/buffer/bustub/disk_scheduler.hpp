...
#include <buffer/bustub/disk_manager.hpp>
...
class DiskScheduler {
public:
  explicit DiskScheduler(DiskManager *disk_manager, int num_worker_ = 48);
  ~DiskScheduler();
   ...
   */
  void StartWorkerThread(int thread_id);
  ...
  auto GetDiskManager() -> DiskManager * { return disk_manager_; }

private:
  /** Pointer to the disk manager. */
  DiskManager *disk_manager_ __attribute__((__unused__));
  /** The number of background threads and the request queues*/
  size_t num_worker_;
