...
template <template <typename> class AdapterType> struct TPCCWorkload {
  ...
  Varchar<16> namePart(Integer id) {
    assert(id < 10);
    Varchar<16> data[] = {"Bar", "OUGHT", "ABLE",  "PRI",   "PRES",
                          "ESE", "ANTI",  "CALLY", "ATION", "EING"};
    return data[id];
  }

  Varchar<16> genName(Integer id) {
    return namePart((id / 100) % 10) || namePart((id / 10) % 10) ||
           namePart(id % 10);
  }
  ...
  Integer nurand(Integer a, Integer x, Integer y, Integer C = 42) {
    ...
    // incorrect: return (((rnd(a) | rnd((y - x + 1) + x)) + 42) % (y - x + 1))
    // + x;
  }
  ...
  void newOrder(Integer w_id, Integer d_id, Integer c_id,
                ...
                const vector<Integer> &supwares, const vector<Integer> &itemids,
                const vector<Integer> &qtys, Timestamp timestamp) {
    Numeric w_tax = warehouse.lookupField({w_id}, &warehouse_t::w_tax);
    Numeric c_discount =
        customer.lookupField({w_id, d_id, c_id}, &customer_t::c_discount);
    Numeric d_tax;
    Integer o_id;
    ...
    for (unsigned i = 0; i < lineNumbers.size(); i++) {
      ...
      stock.update1({supwares[i], itemids[i]}, [&](stock_t &rec) {
        auto &s_quantity =
            rec.s_quantity; // Attention: we also modify s_quantity
        s_quantity =
    ...
    for (unsigned i = 0; i < lineNumbers.size(); i++) {
      ...
      Numeric i_price = item.lookupField(
          {itemid}, &item_t::i_price); // TODO: rollback on miss
      Varchar<24> s_dist;
  ...
  void newOrderRnd(Integer w_id) {
    ...
    for (Integer i = 1; i <= ol_cnt; i++) {
      ...
      if (false && (i == ol_cnt) &&
          (urand(1, 100) == 1)) // invalid item => random
        itemid = 0;
    ...
    newOrder(w_id, d_id, c_id, lineNumbers, supwares, itemids, qtys,
             currentTimestamp());
  }
  // -------------------------------------------------------------------------------------
  void delivery(Integer w_id, Integer carrier_id, Timestamp datetime) {
    for (Integer d_id = 1; d_id <= 10; d_id++) {
      ...
      if (o_id == minInteger) { // Should rarely happen
        cout << "WARNING: delivery tx skipped for warehouse = " << w_id
             << ", district = " << d_id << endl;
        continue;
      }
      // -------------------------------------------------------------------------------------
      ...
      if (manually_handle_isolation_anomalies) {
        ...
        order.scan(
            ...
            [&](const order_t::Key &key, const order_t &rec) {
              if (key.o_w_id == w_id && key.o_d_id == d_id &&
                  key.o_id == o_id) {
      ...
      // -------------------------------------------------------------------------------------
      order.update1({w_id, d_id, o_id},
                    [&](order_t &rec) { rec.o_carrier_id = carrier_id; });
      // -------------------------------------------------------------------------------------
      if (manually_handle_isolation_anomalies) {
        // First check if all orderlines have been inserted, a hack because of
        // the missing transaction and concurrency control
        bool is_safe_to_continue = false;
        orderline.scan(
            {w_id, d_id, o_id, ol_cnt},
            [&](const orderline_t::Key &key, const orderline_t &) {
              if (key.ol_w_id == w_id && key.ol_d_id == d_id &&
                  key.ol_o_id == o_id && key.ol_number == ol_cnt) {
                is_safe_to_continue = true;
              } else {
                is_safe_to_continue = false;
              }
  ...
  void stockLevel(Integer w_id, Integer d_id, Integer threshold) {
    ...
    /*
     * http://www.tpc.org/tpc_documents_current_versions/pdf/tpc-c_v5.11.0.pdf P
  116
  ...
  s_i_id=ol_i_id AND s_quantity < :threshold;
    ...
    for (Integer i_id : items) {
      auto res_s_quantity =
          stock.lookupField({w_id, i_id}, &stock_t::s_quantity);
  ...
  // -------------------------------------------------------------------------------------
  void paymentById(Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id,
                   Integer c_id, Timestamp h_date, Numeric h_amount,
                   Timestamp datetime) {
    Varchar<10> w_name;
    ...
    district.update1({w_id, d_id},
                     [&](district_t &rec) { rec.d_ytd += h_amount; });
    ...
    if (c_credit == "BC") {
      ...
      auto numChars = snprintf(
          c_new_data.data, 500, "| %4d %2d %4d %2d %4d $%7.2f %lu %s%s %s",
          c_id, c_d_id, c_w_id, d_id, w_id, h_amount, h_date,
          w_name.toString().c_str(), d_name.toString().c_str(),
    ...
    Varchar<24> h_new_data =
        Varchar<24>(w_name) || Varchar<24>("    ") || d_name;
  ...
  // -------------------------------------------------------------------------------------
  void paymentByName(Integer w_id, Integer d_id, Integer c_w_id, Integer c_d_id,
                     Varchar<16> c_last, Timestamp h_date, Numeric h_amount,
                     Timestamp datetime) {
    Varchar<10> w_name;
    Varchar<20> w_street_1;
    Varchar<20> w_street_2;
    ...
    customerwdl.scan(
        ...
        [&](const customer_wdl_t::Key &key, const customer_wdl_t &rec) {
          if (key.c_w_id == c_w_id && key.c_d_id == c_d_id &&
              key.c_last == c_last) {
            ids.push_back(rec.c_id);
            return true;
          }
  ...
  void paymentRnd(Integer w_id) {
    ...
    if (urand(1, 100) <= 60) {
      paymentByName(w_id, d_id, c_w_id, c_d_id,
                    genName(getNonUniformRandomLastNameForRun()), h_date,
                    h_amount, currentTimestamp());
    } else {
      paymentById(w_id, d_id, c_w_id, c_d_id, getCustomerID(), h_date, h_amount,
                  currentTimestamp());
    }
  }
  // -------------------------------------------------------------------------------------
public:
  TPCCWorkload(AdapterType<warehouse_t> &w, AdapterType<district_t> &d,
               AdapterType<customer_t> &customer,
               AdapterType<customer_wdl_t> &customerwdl,
               AdapterType<history_t> &history,
               AdapterType<neworder_t> &neworder, AdapterType<order_t> &order,
               AdapterType<order_wdc_t> &order_wdc,
               AdapterType<orderline_t> &orderline, AdapterType<item_t> &item,
               AdapterType<stock_t> &stock, bool order_wdc_index,
               Integer warehouse_count, bool tpcc_remove,
               bool manually_handle_isolation_anomalies = true,
               bool cross_warehouses = true)
      : warehouse(w), district(d), customer(customer), customerwdl(customerwdl),
        history(history), neworder(neworder), order(order),
        order_wdc(order_wdc), orderline(orderline), item(item), stock(stock),
        order_wdc_index(order_wdc_index), warehouseCount(warehouse_count),
        tpcc_remove(tpcc_remove), manually_handle_isolation_anomalies(
  ...
  Integer randomId(Integer fromId, Integer toId) {
    return RandomGenerator::getRand(fromId, toId + 1);
  }
  // [low, high]
  ...
  void loadStock(Integer w_id) {
    for (Integer i = 0; i < ITEMS_NO; i++) {
      ...
      stock.insert({w_id, i + 1},
                   {randomNumeric(10, 100), randomastring<24>(24, 24),
                    randomastring<24>(24, 24), randomastring<24>(24, 24),
                    randomastring<24>(24, 24), randomastring<24>(24, 24),
                    randomastring<24>(24, 24), randomastring<24>(24, 24),
                    randomastring<24>(24, 24), randomastring<24>(24, 24),
                    randomastring<24>(24, 24), 0, 0, 0, s_data});
    }
  }
  // -------------------------------------------------------------------------------------
  void loadDistrinct(Integer w_id) {
    for (Integer i = 1; i < 11; i++) {
      district.insert({w_id, i},
                      {randomastring<10>(6, 10), randomastring<20>(10, 20),
                       randomastring<20>(10, 20), randomastring<20>(10, 20),
                       randomastring<2>(2, 2), randomzip(),
  ...
  void loadCustomer(Integer w_id, Integer d_id) {
    ...
    for (Integer i = 0; i < 3000; i++) {
      ...
      customer.insert({w_id, d_id, i + 1},
                      {c_first, "OE", c_last, randomastring<20>(10, 20),
                       randomastring<20>(10, 20), randomastring<20>(10, 20),
                       randomastring<2>(2, 2), randomzip(),
                       randomnstring(16, 16), now, c_credit, 50000.00,
                       randomNumeric(0.0000, 0.5000), -10.00, 1, 0, 0,
                       randomastring<500>(300, 500)});
      customerwdl.insert({w_id, d_id, c_last, c_first}, {i + 1});
      Integer t_id = (Integer)workerThreadId;
      Integer h_id = (Integer)tpcchistorycounter++;
      history.insert({t_id, h_id}, {i + 1, d_id, w_id, d_id, w_id, now, 10.00,
                                    randomastring<24>(12, 24)});
    }
  }
  // -------------------------------------------------------------------------------------
  void loadOrders(Integer w_id, Integer d_id) {
    ...
    for (Integer o_c_id : c_ids) {
      ...
      order.insert({w_id, d_id, o_id},
                   {o_c_id, now, o_carrier_id, o_ol_cnt, 1});
      if (order_wdc_index) {
  ...
  void loadItem() {
    for (Integer i = 1; i <= ITEMS_NO; i++) {
      ...
      item.insert({i}, {randomId(1, 10000), randomastring<24>(14, 24),
